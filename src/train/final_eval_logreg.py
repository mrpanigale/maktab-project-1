"""In This file we wanna test our logistic regression on test data"""
...
try:
    ...
    print("model is not in right path please run logistic_regression.py first")
    sys.exit(1)
...
result = pd.DataFrame({
                ...
                "model": f"Logistic Regression tr={threshold}",
                "dataset": "Test",
                "precision": [precision],
                "recall": [recall],
                "f1_score": [f1],
                "accuracy": [accuracy],
                "best_C": [model.get_params()["C"]],
            })

result_path = base_dir/"reports"/"final_evaluate"/"logreg"
result_path.mkdir(parents=True, exist_ok=True)
result.to_csv(
    result_path / f"logreg.csv", index=False
)
#=============confusion matrix==============
...
dsp.plot()
plt.title(f"Tree Train | C={model.get_params()["C"]} | TR={threshold}")
plt.savefig(result_path / "logreg_cm.png")
plt.close()
print(f"Logistic Regression threshold {threshold} results saved.")