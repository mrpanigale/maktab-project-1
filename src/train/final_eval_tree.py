"""In This file we wanna test our tree on test data"""
...
try:
    model_path = base_dir / "models"/"decision_tree.pkl"
    model = joblib.load(model_path)
except FileNotFoundError:
    print("model is not in right path please run tree.py first")
    sys.exit(1)
...
result_path = base_dir/"reports"/"final_evaluate"/"tree"
result_path.mkdir(parents=True, exist_ok=True)
result.to_csv(
    result_path / f"tree.csv", index=False
)
#=============confusion matrix==============
...
dsp.plot()
plt.title(f"Test | Depth={model.get_params()["max_depth"]} | TR={threshold}")
plt.savefig(result_path / "tree_cm.png")
plt.close()
